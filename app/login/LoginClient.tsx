...
function LoginForm({ iosApp }: { iosApp: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      {/* Behind a flag, so the button cannot appear on an environment where
          ...
          AND NEVER IN THE iOS APP. App Store Guideline 4.8: an app that offers
          a third-party login must also offer one that limits data to name and
          email, lets the user hide their email, and does no ad tracking - in
          practice, Sign in with Apple. App Review rejected build 8 for this on
          23 September 2026, the day this button went live. Without it the app
          has only Cardtly's own sign-in (password or emailed link), which the
          guideline does not cover. A Microsoft-created account still gets in
          on iOS with the emailed link, since both are the same email. To bring
          the button back in the app, add Sign in with Apple beside it first. */}
      {MS_SSO_ENABLED && !iosApp && (
        <>
...
export default function LoginPageClient({ iosApp }: { iosApp: boolean }) {
  return (
