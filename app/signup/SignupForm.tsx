...
/**
 * THE FORM. Split out of page.tsx so the page itself can be a server
 * component and ask whether this is the iOS app.
 *
 * `iosApp` decides whether the paid option exists AT ALL. App Review rejected
 * this product twice under Guideline 3.1.1, and the rule covers the checkout
 * and every button, link or call to action pointing at it. lib/app-platform
 * makes the point that hiding such a thing in the browser is not enough: the
 * markup is still in the page source, still flashes before hydration, and is
 * still there for a reviewer who looks. So on iOS the price and the paid
 * button are never rendered, and signup behaves exactly as it did before.
 */
export default function SignupForm({ iosApp }: { iosApp: boolean }) {
  // Which button was pressed. Both submit the same form and create the same
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative overflow-hidden">
        ...
        <div className="w-full max-w-sm relative">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest mb-5"
            ...
            {/* The badge has to stay true of the button under it. On iOS the
                only route is the free trial, so "no credit card needed" is
                exactly right. On the web the primary button now asks for R97,
                and a promise of no card directly above it is a contradiction
                the visitor reads before either button. The trial button says
                "no card needed" itself, which is where that claim belongs. */}
            {iosApp ? 'No credit card needed · Live in 2 minutes' : 'Live in 2 minutes · Cancel any time'}
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            {/* ONE BUTTON ON iOS, TWO ON THE WEB. The paid one quotes a price
                and leads to a checkout, so under Guideline 3.1.1 it may not
                exist in the app at all - not hidden, absent. What is left on
                iOS is exactly the button that was here before. */}
            <button
              type="submit"
              onClick={() => setIntent(iosApp ? 'trial' : 'pay')}
              disabled={loading}
            ...
            {!iosApp && (
              <>
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-sm font-semibold transition-all hover:bg-white/5 disabled:opacity-50 disabled:cursor-not-allowed"
                  style={{ color: 'rgba(255,255,255,0.75)', border: '1px solid rgba(255,255,255,0.15)' }}
                >
                  Start 7 days free &mdash; no card needed
                </button>
