...
interface Props {
  ...
  // Position in the first 100 members. If set, renders the small "N/100"
  // badge at the bottom, on every template, without touching each
...
export default function PublicCardView({ card, isPro, isTeamCard, lastActiveAt, founderNumber }: Props) {
  ...
  // just the number: it reads as quietly exclusive without announcing a
  // title. Sits at the bottom so it never competes with the card's own
  // content. env(safe-area-inset-bottom) clears the iOS home indicator.
  const memberBadge = founderNumber ? (
    <div
      className="fixed left-1/2 z-40"
      style={{ bottom: 'calc(env(safe-area-inset-bottom, 0px) + 14px)', transform: 'translateX(-50%)' }}
    >
      <div
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold tracking-wide text-black shadow-lg"
        style={{
          background: 'linear-gradient(135deg, #fcd34d 0%, #fbbf24 50%, #f59e0b 100%)',
          boxShadow: '0 4px 14px rgba(245,158,11,0.35), inset 0 1px 0 rgba(255,255,255,0.6)',
        }}
